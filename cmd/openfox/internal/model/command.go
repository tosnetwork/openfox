...
available list online and pick one), use the 'add' subcommand:
  ...
  openfox model add --help

Examples:
  openfox model                    # Show current default model
  openfox model gpt-5.2           # Set gpt-5.2 as default
  openfox model claude-sonnet-4.6 # Set claude-sonnet-4.6 as default
  openfox model local-model       # Set local VLLM server as default
  openfox model add -b URL -k KEY # Add a model from a custom endpoint
...
func showCurrentModel(cfg *config.Config) {
	...
	fmt.Println("\nTip: 'openfox model add -b URL -k KEY' adds a model from a custom")
	fmt.Println("     OpenAI-compatible endpoint (see 'openfox model add --help').")
}
