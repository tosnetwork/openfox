...
func oracleFixture(t *testing.T, round protocol.Round) (OracleProfile, []ed25519.PrivateKey) {
	...
	return OracleProfile{
		GlobalID: 42, MarketAddress: rawAddress(0x11),
		...
		ArchiveAuthorities: authorities,
	}, keys
}
...
func TestOracleJournalCommitsEvidenceBeforeVoteAndRejectsEquivocation(t *testing.T) {
	...
	_, equivocationErr := journal.PrepareVote(context.ToBOC(), nil, protocol.OutcomeNo, evidence, 11_001)
...
func TestOracleRequiresTwoIndependentSignedArchivesAndExactReviewBase(t *testing.T) {
	...
	_, insufficientArchiveErr := journal.PrepareVote(
		vote.ToBOC(), base.ToBOC(), protocol.OutcomeInvalid, oneArchive, 11_000,
	)
	if insufficientArchiveErr == nil {
	...
	_, tamperedErr := journal.PrepareVote(vote.ToBOC(), base.ToBOC(), protocol.OutcomeInvalid, tampered, 11_000)
	if tamperedErr == nil {
	...
	_, wrongBaseErr := journal.PrepareVote(
		vote.ToBOC(), wrongBase.ToBOC(), protocol.OutcomeInvalid, evidence, 11_000,
	)
	if wrongBaseErr == nil {
		t.Fatal("review vote was detached from its exact base context")
	}
	_, validVoteErr := journal.PrepareVote(vote.ToBOC(), base.ToBOC(), protocol.OutcomeInvalid, evidence, 11_000)
	if validVoteErr != nil {
		t.Fatalf("valid appellate vote rejected: %v", validVoteErr)
	}
}
