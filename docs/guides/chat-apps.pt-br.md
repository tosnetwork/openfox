...
Converse com seu openfox através do Telegram, Discord, WhatsApp, Matrix, QQ, DingTalk, LINE, WeCom, Feishu, Slack, IRC, OneBot, MQTT ou MaixCam
...
| **MaixCam**          | ⭐ Fácil           | Canal de integração de hardware para câmeras AI | [Documentação](../channels/maixcam/README.pt-br.md)                                                            |
| **Pico**             | ⭐ Fácil           | Canal de protocolo nativo OpenFox                    |                                                                                                                  |
...
O OpenFox agora mantém definições de comandos em um registro compartilhado. Na inicialização, o Telegram registrará automaticamente os comandos de bot suportados (por exemplo `/start`, `/help`, `/show`, `/list`, `/use`, `/btw`) para que o menu de comandos e o comportamento em tempo de execução permaneçam sincronizados.
...
Se o registro de comandos falhar (erros transitórios de rede/API), o canal ainda inicia e o OpenFox tenta novamente o registro em segundo plano.
...
Se `session_store_path` estiver vazio, a sessão é armazenada em `<workspace>/whatsapp/`. Execute `openfox gateway`; na primeira execução, escaneie o QR code impresso no terminal com WhatsApp → Dispositivos Vinculados.
...
O OpenFox suporta conexão com sua conta pessoal do WeChat usando a API oficial Tencent iLink.
...
4. Execute `openfox gateway` e abra o QQ para conversar com seu bot
...
* Configure conforme mostrado acima e execute `openfox gateway`
...
Canal de integração projetado especificamente para hardware de câmera AI.
...
O OpenFox suporta três tipos de integração WeCom:
...
O OpenFox se conecta ao Feishu via modo WebSocket/SDK — não é necessário URL de webhook público nem servidor de callback.
...
OneBot é um protocolo aberto para bots QQ. O OpenFox se conecta a qualquer implementação compatível com OneBot v11 (ex.: [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), [NapCat](https://github.com/NapNeko/NapCatQQ)) via WebSocket.
...
Qualquer cliente MQTT pode se comunicar com o OpenFox via broker. Dispositivos ou serviços publicam requisições para o broker; o OpenFox assina, processa e publica as respostas de volta.
...
Nome de usuário e senha em `~/.openfox/.security.yml`:
