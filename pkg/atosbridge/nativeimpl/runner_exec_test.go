...
func TestRunnerExecutionAdapterMapsClaimAndOutcome(t *testing.T) {
	fr := &fakeRunner{outcome: softwarework.Outcome{
		...
		Artifact:        artifactstore.Descriptor{Digest: "sha256:a", MediaType: "application/vnd.atos.software-artifact.v1+tar", SizeBytes: 3072},
		Report:          artifactstore.Descriptor{Digest: "sha256:r", MediaType: "application/vnd.atos.test-report.v1+json", SizeBytes: 355},
		ExitCode:        0,
		CompletedAtUnix: 1786765456,
	}}
	adapter := NewRunnerExecutionAdapter(fr)

	task := atosbridge.Task{
...
func TestRunnerExecutionAdapterPropagatesError(t *testing.T) {
	...
	if _, err := adapter.Execute(context.Background(), atosbridge.Task{}); err == nil {
