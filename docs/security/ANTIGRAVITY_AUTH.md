...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in OpenFox.
...
10. [Creating a New Provider in OpenFox](#creating-a-new-provider-in-openfox)
...
Auth profiles are stored in `~/.openfox/auth.json`:
...
## Creating a New Provider in OpenFox

OpenFox providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/openfox/internal/auth/helpers.go`:
...
### "Token expired"
- Refresh OAuth tokens: `openfox auth login --provider antigravity`
...
- Check auth profile storage: `~/.openfox/auth.json`
- Re-run `openfox auth login --provider antigravity`
