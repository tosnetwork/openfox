<div align="center">
  ...
  <h1>OpenFox : Assistant IA Ultra-Efficace en Go</h1>

  <h3>Matériel à $10 · 10 Mo de RAM · Démarrage en ms · Let's Go, OpenFox!</h3>
  <p>
...
**OpenFox** est un assistant personnel IA ultra-léger inspiré de [NanoBot](https://github.com/HKUDS/nanobot). Il a été entièrement reconstruit en **Go** via un processus d'auto-amorçage (self-bootstrapping) — l'Agent IA lui-même a piloté la migration architecturale et l'optimisation du code.
...
> **Avis de sécurité**
>
> * **PAS DE CRYPTO :** OpenFox n'a **pas** émis de tokens officiels ni de cryptomonnaie. Toute affirmation sur `pump.fun` ou d'autres plateformes de trading est une **arnaque**.
> * **DOMAINE OFFICIEL :** Le **SEUL** site officiel est **[openfox.im](https://openfox.im)**
> * **ATTENTION :** De nombreux domaines `.ai/.org/.com/.net/...` ont été enregistrés par des tiers. Ne leur faites pas confiance.
> * **NOTE :** OpenFox est en développement rapide précoce. Des problèmes de sécurité non résolus peuvent exister. Ne pas déployer en production avant la v1.0.
> * **NOTE :** OpenFox a récemment fusionné de nombreuses PRs. Les builds récents peuvent utiliser 10-20 Mo de RAM. L'optimisation des ressources est prévue après la stabilisation des fonctionnalités.

## 📢 Actualités

2026-05-11 🛒 **LicheeRV-Claw disponible sur AliExpress !** Vous pouvez désormais acheter le LicheeRV-Claw sur [AliExpress](https://www.aliexpress.com/item/1005006519668532.html), ce qui facilite l'essai de OpenFox sur du matériel RISC-V compact.
...
2026-03-31 📱 **Support Android !** OpenFox fonctionne maintenant sur Android ! Téléchargez l'APK sur [openfox.im](https://openfox.im/download)

2026-03-25 🚀 **v0.2.4 publiée !** Refonte de l'architecture Agent (SubTurn, Hooks, Steering, EventBus), intégration WeChat/WeCom, renforcement de la sécurité (.security.yml, filtrage des données sensibles), nouveaux providers (AWS Bedrock, Azure, Xiaomi MiMo), et 35 corrections de bugs. OpenFox a atteint **26K Stars** !

2026-03-17 🚀 **v0.2.3 publiée !** Interface system tray (Windows & Linux), requête de statut des sous-agents (`spawn_status`), rechargement à chaud expérimental du Gateway, sécurisation Cron, et 2 correctifs de sécurité. OpenFox a atteint **25K Stars** !
...
2026-02-26 🎉 OpenFox atteint **20K Stars** en seulement 17 jours ! L'orchestration automatique des channels et les interfaces de capacités sont disponibles.

2026-02-16 🎉 OpenFox dépasse 12K Stars en une semaine ! Rôles de mainteneurs communautaires et [Roadmap](../../ROADMAP.md) officiellement lancés.

2026-02-13 🎉 OpenFox dépasse 5000 Stars en 4 jours ! Roadmap du projet et groupes de développeurs en cours.

2026-02-09 🎉 **OpenFox publié !** Construit en 1 jour pour apporter les Agents IA sur du matériel à $10 avec <10 Mo de RAM. Let's Go, OpenFox !
...
OpenFox peut être déployé sur pratiquement n'importe quel appareil Linux !
...
### Télécharger depuis openfox.im (Recommandé)

Visitez **[openfox.im](https://openfox.im)** — le site officiel détecte automatiquement votre plateforme et fournit un téléchargement en un clic. Pas besoin de choisir manuellement une architecture.

### Télécharger le binaire précompilé

Vous pouvez aussi télécharger le binaire pour votre plateforme depuis la page [GitHub Releases](https://github.com/tosnetwork/openfox/releases).
...
Après téléchargement depuis [openfox.im](https://openfox.im), double-cliquez sur `openfox-launcher` (ou `openfox-launcher.exe` sous Windows). Votre navigateur s'ouvrira automatiquement sur `http://localhost:18800`.
...
Pour la documentation détaillée du WebUI, voir [docs.openfox.im](https://docs.openfox.im).
...
> **Utilisateurs Docker / VM :** Le Gateway écoute sur `127.0.0.1` par défaut. Définissez `OPENFOX_GATEWAY_HOST=0.0.0.0` ou utilisez le flag `-public` pour le rendre accessible depuis l'hôte.
...
macOS peut bloquer `openfox-launcher` au premier lancement car il est téléchargé depuis Internet et n'est pas notarisé via le Mac App Store.

**Étape 1 :** Double-cliquez sur `openfox-launcher`. Un avertissement de sécurité s'affiche :

<p align="center">
<img src="../../assets/macos-gatekeeper-warning.jpg" alt="Avertissement macOS Gatekeeper" width="400">
</p>

> *"openfox-launcher" n'a pas pu être ouvert — Apple n'a pas pu vérifier que "openfox-launcher" ne contient pas de logiciel malveillant susceptible de nuire à votre Mac ou de compromettre votre confidentialité.*
...
Après cette étape unique, `openfox-launcher` s'ouvrira normalement lors des lancements suivants.
...
Donnez une seconde vie à votre téléphone vieux de dix ans ! Transformez-le en assistant IA intelligent avec OpenFox.
...
Téléchargez l'APK depuis [openfox.im](https://openfox.im/download/) et installez-le directement. Pas besoin de Termux !
...
pkg install proot
termux-chroot ./openfox onboard   # chroot fournit une arborescence Linux standard
```
...
Pour les environnements minimaux où seul le binaire principal `openfox` est disponible (sans Launcher UI), vous pouvez tout configurer via la ligne de commande et un fichier de configuration JSON.
...
Cela crée `~/.openfox/config.json` et le répertoire workspace.
...
OpenFox supporte plus de 30 providers LLM via la configuration `model_list`. Utilisez le format `protocole/modèle` :
...
Parlez à votre OpenFox via plus de 17 plateformes de messagerie :
...
> La verbosité des logs est contrôlée par `gateway.log_level` (par défaut : `warn`). Valeurs supportées : `debug`, `info`, `warn`, `error`, `fatal`. Peut aussi être défini via `OPENFOX_LOG_LEVEL`. Voir [Configuration](../guides/configuration.fr.md#niveau-de-log-du-gateway) pour plus de détails.
...
OpenFox peut effectuer des recherches sur le web pour fournir des informations à jour. Configurez dans `tools.web` :
...
OpenFox inclut des outils intégrés pour les opérations sur fichiers, l'exécution de code, la planification et plus encore. Voir [Configuration des outils](../reference/tools_configuration.fr.md) pour les détails.
...
OpenFox supporte nativement [MCP](https://modelcontextprotocol.io/) — connectez n'importe quel serveur MCP pour étendre les capacités de votre Agent avec des outils et sources de données externes.
...
Connectez OpenFox au réseau social des Agents simplement en envoyant un seul message via le CLI ou n'importe quelle application de chat intégrée.
...
| ------------------------- | ---------------------------------------- |
| `openfox onboard`        | Initialiser la config & le workspace     |
| `openfox auth weixin` | Connecter un compte WeChat via QR |
| `openfox agent -m "..."` | Chatter avec l'agent                     |
| `openfox agent`          | Mode chat interactif                     |
| `openfox gateway`        | Démarrer le gateway                      |
| `openfox status`         | Afficher le statut                       |
| `openfox version`        | Afficher les informations de version     |
| `openfox model`          | Voir ou changer le modèle par défaut     |
| `openfox cron list`      | Lister toutes les tâches planifiées      |
| `openfox cron add ...`   | Ajouter une tâche planifiée              |
| `openfox cron disable`   | Désactiver une tâche planifiée           |
| `openfox cron remove`    | Supprimer une tâche planifiée            |
| `openfox skills list`    | Lister les Skills installées             |
| `openfox skills install` | Installer une Skill                      |
| `openfox migrate`        | Migrer les données depuis d'anciennes versions |
| `openfox auth login`     | S'authentifier auprès des providers      |

### ⏰ Tâches planifiées / Rappels

OpenFox supporte les rappels planifiés et les tâches récurrentes via l'outil `cron` :
...
Consultez notre [Roadmap communautaire](https://github.com/tosnetwork/openfox/issues/988) pour les directives.
