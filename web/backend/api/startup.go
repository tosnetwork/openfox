...
const (
	autoStartEntryName = "OpenFoxLauncher"
	launchAgentLabel   = "io.openfox.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "openfox-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=OpenFox Web",
			"Comment=Start OpenFox Web on login",
