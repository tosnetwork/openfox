...
type CompositeTaskTransport struct {
	routes map[atosbridge.Transport]atosbridge.TaskTransport
}
...
func NewCompositeTaskTransport(routes map[atosbridge.Transport]atosbridge.TaskTransport) (*CompositeTaskTransport, error) {
	if len(routes) == 0 {
		return nil, errors.New("nativeimpl: composite transport needs at least one route")
	}
	owned := make(map[atosbridge.Transport]atosbridge.TaskTransport, len(routes))
...
func (t *CompositeTaskTransport) Dispatch(ctx context.Context, transport atosbridge.Transport, task atosbridge.Task) error {
...
var _ atosbridge.TaskTransport = (*CompositeTaskTransport)(nil)
