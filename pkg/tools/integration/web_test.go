...
// with cf-mitigated: challenge triggers a retry using the honest openfox User-Agent,
...
func TestWebFetchTool_CloudflareChallenge_RetryWithHonestUA(t *testing.T) {
	...
	// Second request must use the honest openfox user agent
	if !strings.Contains(receivedUAs[1], "openfox") {
		t.Errorf("retry request UA = %q, want it to contain 'openfox'", receivedUAs[1])
	}
}
