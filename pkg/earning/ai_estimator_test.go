...
func TestLLMEconomicEstimatorProducesEvidenceButNoAuthority(t *testing.T) {
	...
	if _, err := (LLMEconomicEstimator{Provider: bad, Now: func() time.Time { return now }}).Estimate(
		context.Background(), intent, inventory,
	); err == nil {
	...
	if _, err := (LLMEconomicEstimator{
		Provider: missingDisposition,
		Now:      func() time.Time { return now },
	}).Estimate(context.Background(), intent, inventory); err == nil {
...
func TestLLMEconomicEstimatorDeclineIsAValidFailClosedDecision(t *testing.T) {
	...
	decision, err := EvaluateEconomics(
		estimate,
		EconomicPolicy{MinimumExpectedProfitAtomic: "1", MaximumLossAtomic: "100"},
		now,
	)
