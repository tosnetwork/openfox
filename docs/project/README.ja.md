<div align="center">
  ...
  <h1>OpenFox: Go で書かれた超効率 AI アシスタント</h1>

  <h3>$10 ハードウェア · 10MB RAM · ms 起動 · Let's Go, OpenFox!</h3>
  <p>
...
**OpenFox** は [NanoBot](https://github.com/HKUDS/nanobot) にインスパイアされた超軽量パーソナル AI アシスタントです。**Go** でゼロからリビルドされ、「セルフブートストラッピング」プロセスで構築されました — AI Agent 自身がアーキテクチャの移行とコード最適化を推進しました。
...
> **セキュリティに関する注意**
>
> * **暗号通貨なし:** OpenFox には公式トークン/コインは**一切ありません**。`pump.fun` やその他の取引プラットフォームでの主張はすべて**詐欺**です。
> * **公式ドメイン:** **唯一**の公式サイトは **[openfox.im](https://openfox.im)** です。
> * **注意:** 多くの `.ai/.org/.com/.net/...` ドメインは第三者によって登録されています。信頼しないでください。
> * **注記:** OpenFox は初期開発段階にあり、未解決のネットワークセキュリティ問題がある可能性があります。v1.0 リリース前に本番環境へのデプロイは避けてください。
> * **注記:** OpenFox は最近多くの PR をマージしており、最新バージョンではメモリフットプリントが大きくなる場合があります（10〜20MB）。機能セットが安定次第、リソース最適化を優先する予定です。

## 📢 ニュース

2026-05-11 🛒 **LicheeRV-Claw が AliExpress で購入可能に！** [AliExpress](https://www.aliexpress.com/item/1005006519668532.html) から LicheeRV-Claw を購入できるようになり、コンパクトな RISC-V ハードウェアで OpenFox を試しやすくなりました。
...
2026-03-31 📱 **Android サポート！** OpenFoxがAndroidで動作！APKは[openfox.im](https://openfox.im/download)からダウンロード

2026-03-25 🚀 **v0.2.4 リリース！** Agent アーキテクチャ全面刷新（SubTurn、Hooks、Steering、EventBus）、WeChat/WeCom 統合、セキュリティ強化（.security.yml、機密データフィルタリング）、新プロバイダー（AWS Bedrock、Azure、Xiaomi MiMo）、35 件のバグ修正。OpenFox **26K ⭐** 達成！

2026-03-17 🚀 **v0.2.3 リリース！** システムトレイ UI（Windows & Linux）、サブエージェントステータス追跡（`spawn_status`）、実験的 Gateway ホットリロード、cron セキュリティゲート、セキュリティ修正 2 件。OpenFox **25K ⭐** 達成！
...
2026-02-26 🎉 OpenFox がわずか 17 日で **20K スター** 達成！Channel 自動オーケストレーションとケイパビリティインターフェースが実装されました。

2026-02-16 🎉 OpenFox が 1 週間で 12K スター達成！コミュニティメンテナーの役割と[ロードマップ](../../ROADMAP.md)が正式に公開されました。

2026-02-13 🎉 OpenFox が 4 日間で 5000 スター達成！プロジェクトロードマップと開発者グループの準備が進行中。

2026-02-09 🎉 **OpenFox リリース！** $10 ハードウェアで 10MB 未満の RAM で動く AI Agent を 1 日で構築。Let's Go, OpenFox!
...
OpenFox はほぼすべての Linux デバイスにデプロイできます！
...
### openfox.im からダウンロード（推奨）

**[openfox.im](https://openfox.im)** にアクセス — 公式サイトがプラットフォームを自動検出し、ワンクリックでダウンロードできます。アーキテクチャを手動で選ぶ必要はありません。

### プリコンパイル済みバイナリをダウンロード

または、[GitHub Releases](https://github.com/tosnetwork/openfox/releases) ページからプラットフォームに合ったバイナリをダウンロードしてください。
...
[openfox.im](https://openfox.im) からダウンロード後、`openfox-launcher`（Windows では `openfox-launcher.exe`）をダブルクリックしてください。ブラウザが自動的に `http://localhost:18800` を開きます。
...
WebUI の詳細なドキュメントは [docs.openfox.im](https://docs.openfox.im) を参照してください。
...
> **Docker / VM ユーザー:** Gateway はデフォルトで `127.0.0.1` でリッスンします。ホストからアクセスできるようにするには `OPENFOX_GATEWAY_HOST=0.0.0.0` を設定するか、`-public` フラグを使用してください。
...
`openfox-launcher` はインターネットからダウンロードされ、Mac App Store を通じて公証されていないため、macOS が初回起動時にブロックする場合があります。

**ステップ 1：** `openfox-launcher` をダブルクリックすると、セキュリティ警告が表示されます：

<p align="center">
<img src="../../assets/macos-gatekeeper-warning.jpg" alt="macOS Gatekeeper 警告" width="400">
</p>

> *"openfox-launcher" は開けません — "openfox-launcher" がMacに害を与えたりプライバシーを侵害するマルウェアを含まないことをAppleは確認できません。*
...
10 年前のスマホに第二の人生を！OpenFox でスマート AI アシスタントに変身させましょう。
...
[openfox.im](https://openfox.im/download/) から APK をダウンロードして直接インストール。Termux 不要！
...
pkg install proot
termux-chroot ./openfox onboard   # chroot で標準的な Linux ファイルシステムレイアウトを提供
```
...
`openfox` コアバイナリのみが利用可能な最小環境（Launcher UI なし）では、コマンドラインと JSON 設定ファイルですべてを設定できます。
...
`~/.openfox/config.json` とワークスペースディレクトリが作成されます。
...
OpenFox は `model_list` 設定を通じて 30 以上の LLM Provider をサポートしています。`protocol/model` 形式を使用してください：
...
17 以上のメッセージングプラットフォームで OpenFox と会話できます：
...
> ログの詳細度は `gateway.log_level` で制御します（デフォルト：`warn`）。サポートされる値：`debug`、`info`、`warn`、`error`、`fatal`。`OPENFOX_LOG_LEVEL` 環境変数でも設定可能です。詳細は[設定ガイド](../guides/configuration.ja.md#gateway-ログレベル)を参照してください。
...
OpenFox は最新情報を提供するために Web を検索できます。`tools.web` で設定してください：
...
OpenFox にはファイル操作、コード実行、スケジューリングなどの組み込みツールが含まれています。詳細は [ツール設定](../reference/tools_configuration.ja.md) を参照してください。
...
OpenFox は [MCP](https://modelcontextprotocol.io/) をネイティブサポートしています — 任意の MCP サーバーに接続して、外部ツールやデータソースで Agent の機能を拡張できます。
...
CLI または統合チャットアプリからメッセージを 1 つ送るだけで、OpenFox をエージェントソーシャルネットワークに接続できます。
...
| コマンド                  | 説明                           |
| ------------------------- | ------------------------------ |
| `openfox onboard`        | 設定＆ワークスペースの初期化     |
| `openfox auth weixin` | WeChat アカウントを QR で接続 |
| `openfox agent -m "..."` | Agent とチャット                |
| `openfox agent`          | インタラクティブチャットモード   |
| `openfox gateway`        | Gateway を起動                  |
| `openfox status`         | ステータスを表示                |
| `openfox version`        | バージョン情報を表示            |
| `openfox model`          | デフォルトモデルの表示・切替    |
| `openfox cron list`      | スケジュールジョブ一覧          |
| `openfox cron add ...`   | スケジュールジョブを追加         |
| `openfox cron disable`   | スケジュールジョブを無効化       |
| `openfox cron remove`    | スケジュールジョブを削除         |
| `openfox skills list`    | インストール済み Skill 一覧      |
| `openfox skills install` | Skill をインストール             |
| `openfox migrate`        | 旧バージョンからデータを移行     |
| `openfox auth login`     | Provider への認証               |

### ⏰ スケジュールタスク / リマインダー

OpenFox は `cron` ツールによるスケジュールリマインダーと定期タスクをサポートしています：
...
[コミュニティロードマップ](https://github.com/tosnetwork/openfox/issues/988)をご覧ください。
