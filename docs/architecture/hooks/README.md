...
If you are developing OpenFox itself rather than only validating the protocol, continue with the Go in-process example as well.
...
- Python process example
  Best for understanding the `JSON-RPC over stdio` protocol and verifying the message flow between OpenFox and an external process
...
Note: `OPENFOX_HOOK_LOG_FILE` has no default. If you do not set it, the script does not write any file logs.
...
- OpenFox starts the external process
...
The host does not currently accept new RPCs initiated by the process hook. In practice, that means an external hook can only respond to OpenFox calls; it cannot call back into the host to send channel messages.
