...
# 🖥️ OpenFox ハードウェア互換性リスト

OpenFox はほぼすべての Linux デバイスで動作します。このページでは、検証済みのチップ、製品、開発ボードを記録しています。
...
OpenFox でテスト済みのコンシューマー製品、ルーター、産業用デバイス。
...
| 2025 | NanoKVM-Pro | ARM64 (A53) | AX630C | 1GB | プロ IP-KVM |
| 2026 | MaixCAM2 | ARM64 (A53) | AX630C | 1/4GB | 4K AI カメラ |
...
1GB 以上の RAM を搭載した ARM64 Android スマートフォン（2015年以降）。[Termux](https://github.com/termux/termux-app) をインストールし、`proot` を使用して OpenFox を実行します。
