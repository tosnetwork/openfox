...
func validAgentRelayEarningSettings(role string) EarningSettings {
	...
	settings := EarningSettings{Enabled: true, Mode: "policy-gated", StateDir: "/var/lib/openfox/earning",
		...
		TrustedCapability: TrustedCapabilitySettings{
			ProjectionDirectory:           "/var/lib/openfox/trusted-capability",
			PublisherObservationDirectory: "/var/lib/openfox-publisher-observations",
			ExecutionBundleDirectory:      "/var/lib/openfox-execution-bundles",
			SinkJournalDirectory:          "/var/lib/openfox-sink-journals",
			ControlAuthorityEndpoint:      "https://control-authority.example/v1/openfox-control-authority",
			ControlAuthorityTokenFile:     "/etc/openfox/control-authority.token",
			ControlAuthorityPublicKey:     "ed25519:" + strings.Repeat("c", 64),
		},
