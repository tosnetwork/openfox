...
func (f aliasClientFunc) ResolveDNSAlias(ctx context.Context, request *nativev1.ResolveDNSAliasRequest) (*nativev1.ResolveDNSAliasResponse, error) {
...
func cloneAlias(value *nativev1.ResolveDNSAliasResponse) *nativev1.ResolveDNSAliasResponse {
	...
	state := &nativev1.NativeStateV1{Network: value.NativeState.Network, Reference: &nativev1.ChainReference{
		Account: value.NativeState.Reference.Account, FinalizedCheckpoint: value.NativeState.Reference.FinalizedCheckpoint}}
	if value.NativeState.GetAgent() != nil {
		state.State = &nativev1.NativeStateV1_Agent{Agent: &nativev1.AgentStateV1{AgentId: value.NativeState.GetAgent().AgentId}}
	}
	return &nativev1.ResolveDNSAliasResponse{CanonicalName: value.CanonicalName, Kind: value.Kind, CategoryHash: value.CategoryHash,
		NativeObjectId: value.NativeObjectId, NativeState: state, ResolvedAccount: value.ResolvedAccount,
		Checkpoint: &nativev1.DNSCheckpointV1{Workchain: value.Checkpoint.Workchain, Sequence: value.Checkpoint.Sequence,
			RootHash: append([]byte(nil), value.Checkpoint.RootHash...), FileHash: append([]byte(nil), value.Checkpoint.FileHash...),
			GenerationUnixSeconds: value.Checkpoint.GenerationUnixSeconds},
		Lifecycle: &nativev1.DNSLifecycleV1{AuctionEndUnixSeconds: value.Lifecycle.AuctionEndUnixSeconds,
...
func testAliasNetwork() *nativev1.NetworkDomain {
	return &nativev1.NetworkDomain{NetworkId: "tos-test", GenesisRootHash: strings.Repeat("1", 64), GenesisFileHash: strings.Repeat("2", 64)}
}
