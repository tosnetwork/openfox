...
// the extracted atosbridge phase functions.
...
func TestMobilePurchasePhaseVectors(t *testing.T) {
	...
	if vectors.Schema != "atos.native.mobile-buyer-purchase-phase.v1" ||
	...
	for _, c := range vectors.Resume {
		phase := atosbridge.Phase(c.Phase)
		if got := atosbridge.CanAcquireFundingLease(phase); got != c.CanAcquireLease {
			t.Fatalf("phase %s: CanAcquireFundingLease got %v, want %v", c.Phase, got, c.CanAcquireLease)
		}
		if got := string(atosbridge.ResumeActionFor(phase)); got != c.ResumeAction {
			t.Fatalf("phase %s: ResumeActionFor got %q, want %q", c.Phase, got, c.ResumeAction)
		}
	}

	for _, c := range vectors.Transitions {
		if got := atosbridge.CanAdvance(atosbridge.Phase(c.From), atosbridge.Phase(c.To)); got != c.CanAdvance {
