...
// e2eResolver implements servicebridge.NativeResolver. ResolveEscrow reads the
...
// e2eReceipts implements servicebridge.ReceiptVerifier for the buyer. The buyer
...
type inProcessTransport struct {
	handle func(context.Context, servicebridge.Task) error
}

func (t inProcessTransport) Dispatch(ctx context.Context, _ servicebridge.Transport, task servicebridge.Task) error {
...
func TestBridgeClosesTheFullPaidLoop(t *testing.T) {
	...
	dupErr := transport.handle(context.Background(), servicebridge.Task{
