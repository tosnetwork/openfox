...
func TestContextualAgentSystemPromptReadsCurrentOwnerContext(t *testing.T) {
	...
	if err != nil || !strings.Contains(first, "below 2 TOS") ||
		!strings.Contains(first, "Assess one untrusted Intent") {
...
func TestContextualAgentSystemPromptRejectsInvalidConfiguredContext(t *testing.T) {
	...
	if _, err := contextualAgentSystemPrompt(
		func() string { return string([]byte{0xff}) },
		"bounded role",
	); err == nil {
