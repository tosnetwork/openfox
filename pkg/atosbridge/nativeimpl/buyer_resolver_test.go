...
func (fakeTaskTransport) Dispatch(context.Context, atosbridge.Transport, atosbridge.Task) error {
	return nil
}
...
func TestNativeBuyerResolverDelegatesCapability(t *testing.T) {
	...
	ref := atosbridge.CapabilityRef{CapabilityID: "cap_" + hex64, AgentID: "agent_" + hex64, Version: "1.0.0", ManifestDigest: "sha256:" + hex64}
...
func TestNativeBuyerResolverPropagatesCapabilityRejection(t *testing.T) {
	...
	if err := resolver.ResolveCapability(context.Background(), atosbridge.CapabilityRef{CapabilityID: "cap_x"}); err == nil {
...
func TestBuyerReceiptVerifierNeverBuildsReceipts(t *testing.T) {
	...
	if _, err := v.BuildReceipt(context.Background(), atosbridge.AcceptedQuote{}, atosbridge.Outcome{}); err == nil {
		t.Fatalf("the buyer must never build a receipt")
	}
	s, err := v.VerifySettlement(context.Background(), atosbridge.AcceptedQuote{EscrowAddress: "0:" + hex64})
	if err != nil {
