...
// test proves they match the real atosbridge.PolicyEngine, mapping its typed
...
func reasonFor(err error) string {
	...
	case err == nil:
		return "ok"
	case errors.Is(err, atosbridge.ErrPolicyInvalid):
		return "policy_invalid"
	case errors.Is(err, atosbridge.ErrPolicyExpired):
		return "policy_expired"
	case errors.Is(err, atosbridge.ErrAssetNotAllowed):
		return "asset_not_allowed"
	case errors.Is(err, atosbridge.ErrCapabilityNotAllowed):
		return "capability_not_allowed"
	case errors.Is(err, atosbridge.ErrOverPurchaseLimit):
		return "over_purchase_limit"
	case errors.Is(err, atosbridge.ErrOverDailyBudget):
		return "over_daily_budget"
	case errors.Is(err, atosbridge.ErrManualConfirmation):
...
func buildPolicy(t *testing.T, f policyFields) atosbridge.SpendingPolicy {
	t.Helper()
	...
	return atosbridge.SpendingPolicy{
		Asset:             atosbridge.AssetIdentity{Master: *f.AssetMaster, WalletCodeHash: *f.AssetWalletCodeHash},
		...
		ConfirmationMode:  atosbridge.ConfirmationMode(*f.ConfirmationMode),
		OwnerSignature:    signature,
	}
}

func buildProposal(t *testing.T, f proposalFields) atosbridge.QuoteProposal {
	t.Helper()
	return atosbridge.QuoteProposal{
		Capability:      atosbridge.CapabilityRef{CapabilityID: *f.CapabilityID},
		Asset:           atosbridge.AssetIdentity{Master: *f.AssetMaster, WalletCodeHash: *f.AssetWalletCodeHash},
...
func TestMobileSpendingPolicyVectors(t *testing.T) {
	...
	if vectors.Schema != "atos.native.mobile-buyer-spending-policy.v1" || len(vectors.Cases) == 0 {
	...
	for _, c := range vectors.Cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			err := (atosbridge.PolicyEngine{}).Authorize(policy, proposal, spent, now)
