...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via OPENFOX_KEY_PASSPHRASE
...
//  2. OPENFOX_SSH_KEY_PATH env var
//  3. ~/.ssh/openfox_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "OPENFOX_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading OPENFOX_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "OPENFOX_SSH_KEY_PATH"

// openfoxHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const openfoxHome = "OPENFOX_HOME"

const (
	FileScheme = "file://"
	EncScheme  = "enc://"

	hkdfInfo = "openfox-credential-v1"
	saltLen  = 16
	nonceLen = 12
	keyLen   = 32
)
...
//
// passphrase is required (OPENFOX_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// OPENFOX_SSH_KEY_PATH env var or ~/.ssh/openfox_ed25519.key.
...
//   - exact match with OPENFOX_SSH_KEY_PATH env var
//   - within the OPENFOX_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with OPENFOX_SSH_KEY_PATH.
	...
	// Within OPENFOX_HOME.
	if picoHome := os.Getenv(openfoxHome); picoHome != "" {
...
// Final key: HKDF-SHA256(ikm, salt, info="openfox-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set OPENFOX_SSH_KEY_PATH or place key at ~/.ssh/openfox_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (OPENFOX_SSH_KEY_PATH, OPENFOX_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. OPENFOX_SSH_KEY_PATH env var
//  3. ~/.ssh/openfox_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the openfox-specific SSH key path if it exists.
