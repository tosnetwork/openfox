...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("openfox")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@openfox hello", want: true},
		{text: "hi @openfox:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bopenfox\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@openfox:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@openfox:matrix.org hello", want: "hello"},
		{in: "@openfox, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@openfox:matrix.org")},
				},
			},
			want: true,
		},
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@openfox:matrix.org">OpenFox</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40openfox%3Amatrix.org">OpenFox</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@openfox:matrix.test"), "")
	if err != nil {
