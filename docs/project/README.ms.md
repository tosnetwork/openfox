...
<h1>OpenFox: Pembantu AI Ultra-Cekap dalam Go</h1>
...
**OpenFox** adalah pembantu AI peribadi ultra-ringan yang terinspirasi oleh [NanoBot](https://github.com/HKUDS/nanobot). Ia dibina semula dari awal dalam **Go** melalui proses "self-bootstrapping" — AI Agent itu sendiri yang memacu migrasi seni bina dan pengoptimuman kod.
...
> **Notis Keselamatan**
>
> * **TIADA KRIPTO:** OpenFox **tidak** mengeluarkan sebarang token atau mata wang kripto rasmi. Semua tuntutan di `pump.fun` atau platform dagangan lain adalah **penipuan**.
> * **DOMAIN RASMI:** Satu-satunya laman web rasmi ialah **[openfox.im](https://openfox.im)**
> * **BERHATI-HATI:** Banyak domain `.ai/.org/.com/.net/...` telah didaftarkan oleh pihak ketiga. Jangan percayai mereka.
> * **NOTA:** OpenFox dalam pembangunan pesat awal. Mungkin terdapat isu keselamatan yang belum diselesaikan. Jangan deploy ke pengeluaran sebelum v1.0.


## 📢 Berita

2026-05-11 🛒 **LicheeRV-Claw tersedia di AliExpress!** Anda kini boleh membeli LicheeRV-Claw di [AliExpress](https://www.aliexpress.com/item/1005006519668532.html), menjadikannya lebih mudah untuk mencuba OpenFox pada perkakasan RISC-V yang kompak.
...
2026-03-31 📱 **Sokongan Android!** OpenFox sekarang berjalan di Android! Muat turun APK di [openfox.im](https://openfox.im/download)

2026-03-25 🚀 **v0.2.4 Dikeluarkan!** Penstrukturan semula seni bina Agent (SubTurn, Hooks, Steering, EventBus), integrasi WeChat/WeCom, penguatan keselamatan (.security.yml, penapisan data sensitif), penyedia baharu (AWS Bedrock, Azure, Xiaomi MiMo), dan 35 pembetulan pepijat. OpenFox mencapai **26K Stars**!

2026-03-17 🚀 **v0.2.3 Dikeluarkan!** UI dulang sistem (Windows & Linux), pertanyaan status sub-agent (`spawn_status`), muat semula panas Gateway eksperimental, kawalan keselamatan Cron, dan 2 pembetulan keselamatan. OpenFox mencapai **25K Stars**!
...
2026-02-26 🎉 OpenFox mencapai **20K Stars** hanya dalam 17 hari! Orkestrasi saluran automatik dan antara muka keupayaan kini aktif.

2026-02-16 🎉 OpenFox melepasi 12K Stars dalam seminggu! Peranan penyelenggara komuniti dan [Peta Jalan](../../ROADMAP.md) dilancarkan secara rasmi.

2026-02-13 🎉 OpenFox melepasi 5000 Stars dalam 4 hari! Peta jalan projek dan kumpulan pembangun sedang dalam proses.

2026-02-09 🎉 **OpenFox Dikeluarkan!** Dibina dalam 1 hari untuk membawa AI Agent ke perkakasan $10 dengan RAM <10MB. Jom, OpenFox!
...
<p align="center">
<img src="../../assets/hardware-banner.jpg" alt="Keserasian Perkakasan OpenFox" width="100%">
</p>
...
OpenFox boleh digunakan pada hampir mana-mana peranti Linux!
...
### Muat turun dari openfox.im (Disyorkan)

Lawati **[openfox.im](https://openfox.im)** — laman web rasmi mengesan platform anda secara automatik dan menyediakan muat turun satu klik.

### Muat turun binari pra-kompil

Muat turun binari untuk platform anda dari halaman [GitHub Releases](https://github.com/tosnetwork/openfox/releases).
...
Selepas memuat turun dari [openfox.im](https://openfox.im), klik dua kali `openfox-launcher` (atau `openfox-launcher.exe` pada Windows). Pelayar anda akan dibuka secara automatik di `http://localhost:18800`.
...
Untuk dokumentasi WebUI terperinci, lihat [docs.openfox.im](https://docs.openfox.im).
...
> **Pengguna Docker / VM:** Gateway mendengar pada `127.0.0.1` secara lalai. Tetapkan `OPENFOX_GATEWAY_HOST=0.0.0.0` atau gunakan bendera `-public` untuk membolehkan akses dari hos.
...
macOS mungkin menyekat `openfox-launcher` pada pelancaran pertama kerana ia dimuat turun dari internet dan tidak disahkan melalui Mac App Store.

**Langkah 1:** Klik dua kali `openfox-launcher`. Anda akan melihat amaran keselamatan:

<p align="center">
<img src="../../assets/macos-gatekeeper-warning.jpg" alt="Amaran macOS Gatekeeper" width="400">
</p>

> *"openfox-launcher" Tidak Dibuka — Apple tidak dapat mengesahkan "openfox-launcher" bebas daripada perisian hasad yang mungkin membahayakan Mac anda atau menjejaskan privasi anda.*
...
Selepas langkah sekali ini, `openfox-launcher` akan dibuka secara normal pada pelancaran seterusnya.

</details>

### 📱 Android

Berikan telefon lama anda kehidupan baru! Jadikannya Pembantu AI pintar dengan OpenFox.
...
Muat turun APK dari [openfox.im](https://openfox.im/download/) dan pasang secara langsung. Tiada Termux diperlukan!
...
pkg install proot
termux-chroot ./openfox onboard   # chroot menyediakan susun atur sistem fail Linux standard
```

Kemudian ikuti bahagian Pelancar Terminal di bawah untuk melengkapkan konfigurasi.

<img src="../../assets/termux.jpg" alt="OpenFox pada Termux" width="512">

Untuk persekitaran minimal di mana hanya binari teras `openfox` tersedia (tiada UI Pelancar), anda boleh mengkonfigurasi semua melalui baris arahan dan fail konfigurasi JSON.
...
Ini mencipta `~/.openfox/config.json` dan direktori ruang kerja.

**2. Konfigurasikan** (`~/.openfox/config.json`)
...
OpenFox menyokong 30+ penyedia LLM melalui konfigurasi `model_list`. Gunakan format `protokol/model`:
...
Bercakap dengan OpenFox anda melalui 17+ platform pemesejan:
...
> Tahap perincian log dikawal oleh `gateway.log_level` (lalai: `warn`). Nilai yang disokong: `debug`, `info`, `warn`, `error`, `fatal`. Boleh juga ditetapkan melalui `OPENFOX_LOG_LEVEL`. Lihat [Konfigurasi](../guides/configuration.ms.md#gateway-log-level) untuk butiran.
...
OpenFox boleh mencari web untuk menyediakan maklumat terkini. Konfigurasikan dalam `tools.web`:
...
OpenFox menyertakan alat terbina dalam untuk operasi fail, pelaksanaan kod, penjadualan, dan banyak lagi. Lihat [Konfigurasi Alat](../reference/tools_configuration.md) untuk butiran.
...
openfox skills install <nama-kemahiran>
```
...
OpenFox menyokong [MCP](https://modelcontextprotocol.io/) secara natif — sambungkan mana-mana pelayan MCP untuk melanjutkan keupayaan Agent anda dengan alat dan sumber data luaran.
...
Sambungkan OpenFox ke Rangkaian Sosial Agent dengan menghantar satu mesej melalui CLI atau mana-mana Aplikasi Sembang yang disepadukan.
...
| Arahan | Penerangan |
| ------ | ---------- |
| `openfox onboard` | Mulakan konfigurasi & ruang kerja |
| `openfox auth weixin` | Sambungkan akaun WeChat melalui QR |
| `openfox agent -m "..."` | Sembang dengan agent |
| `openfox agent` | Mod sembang interaktif |
| `openfox gateway` | Mulakan gateway |
| `openfox status` | Tunjukkan status |
| `openfox version` | Tunjukkan maklumat versi |
| `openfox model` | Lihat atau tukar model lalai |
| `openfox cron list` | Senaraikan semua kerja berjadual |
| `openfox cron add ...` | Tambah kerja berjadual |
| `openfox cron disable` | Lumpuhkan kerja berjadual |
| `openfox cron remove` | Buang kerja berjadual |
| `openfox skills list` | Senaraikan kemahiran yang dipasang |
| `openfox skills install` | Pasang kemahiran |
| `openfox migrate` | Migrasi data dari versi lama |
| `openfox auth login` | Sahkan dengan penyedia |

### ⏰ Tugasan Berjadual / Peringatan

OpenFox menyokong peringatan berjadual dan tugasan berulang melalui alat `cron`:
...
Lihat [Peta Jalan Komuniti](https://github.com/tosnetwork/openfox/issues/988) untuk panduan.
