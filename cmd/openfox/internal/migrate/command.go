...
import (
	...
	"github.com/tosnetwork/openfox/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to openfox",
		Args:  cobra.NoArgs,
		Example: `  openfox migrate
  openfox migrate --from openclaw
  openfox migrate --dry-run
  openfox migrate --refresh
  openfox migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.openfox)")
