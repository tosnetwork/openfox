---
name: openfox-agent
description: "Configure, extend, debug, or contribute to OpenFox itself. Use when the task is about OpenFox CLI commands, config.json, gateway, auth, models, skills, MCP servers, cron, routing, sessions, self-evolution, built-in slash commands, or repository internals. Use OpenFox-native workflows, terminology, paths, and configuration."
metadata: {"nanobot":{"emoji":"🦞"}}
---

# OpenFox Agent

OpenFox is a lightweight personal AI assistant and agent framework with a native CLI, chat gateway, MCP integration, installable skills, session routing, scheduled jobs, and self-evolution.

Use this skill when the job is about **OpenFox itself**: onboarding, configuration, debugging, adding features, extending the CLI, changing routing/session behavior, working on skills or MCP support, or contributing to this repository.

## Operating Stance

When this skill is active, stay fully subordinate to OpenFox's real architecture:

- Prefer OpenFox commands, config keys, workspace layout, and docs.
- Follow OpenFox source and docs for behavior, naming, and workflows.
- Treat repository code and checked-in docs as the source of truth.

## Quick Start

```bash
# Initialize ~/.openfox/config.json and ~/.openfox/workspace
openfox onboard

# Authenticate a provider
openfox auth login --provider openai

# Inspect or switch the default model
openfox model
openfox model my-default-model
...
```bash
openfox onboard
openfox agent [-m MESSAGE] [--session KEY] [--model MODEL] [--debug]
openfox gateway [--debug] [--no-truncate] [--allow-empty] [--host HOST]
openfox status
openfox version
openfox migrate
```
...
openfox model
openfox model <model_name>

# Add a model from an OpenAI-compatible endpoint
openfox model add --api-base URL --api-key KEY
openfox model add -b http://localhost:8000/v1 -k dummy -m my-model -n local

# Reset config to factory defaults (preserves sensitive keys)
openfox config reset
openfox config reset --force
```

Notes:

- There is **no** general `openfox config edit` or `openfox config set`.
- Advanced edits are usually done by editing `~/.openfox/config.json` directly.

### Authentication

```bash
openfox auth login --provider openai
openfox auth login --provider anthropic --setup-token
openfox auth login --provider antigravity --device-code
openfox auth models
openfox auth status
openfox auth logout --provider openai
openfox auth weixin
...
```bash
openfox skills list
openfox skills show <name>
openfox skills search "query"
openfox skills install owner/repo/path
openfox skills install --registry clawhub <slug>
openfox skills remove <name>
openfox skills list-builtin
openfox skills install-builtin
```

Skill loading priority is:

1. `~/.openfox/workspace/skills`
2. `~/.openfox/skills`
...
openfox mcp add --deferred github --env-file .env.github -- npx -y @modelcontextprotocol/server-github
openfox mcp list
openfox mcp list --status
openfox mcp show github
openfox mcp test github
openfox mcp edit
openfox mcp remove github
```

Notes:

- `openfox mcp` is a **configuration manager** for `tools.mcp.servers`.
...
openfox cron enable <job-id>
openfox cron disable <job-id>
openfox cron remove <job-id>
```

Notes:

- The current CLI `openfox cron add` supports recurring jobs only: `--every` or `--cron`.
- One-shot `at_seconds` jobs exist in the cron system, but not as a first-class CLI flag today.

## Providers

OpenFox supports 30+ LLM providers through `model_list`.

Credential patterns in OpenFox are:

- `model_list[].api_keys` for most hosted APIs
- `openfox auth login --provider ...` for the built-in auth helper flows (`openai`, `anthropic`, `antigravity`)
...
`openfox auth login` does **not** cover every provider. For most providers, the normal path is adding a `model_list` entry with `provider`, `model`, and `api_keys`.

### Common Provider Matrix

| Provider | `provider` value | Auth path in OpenFox |
| --- | --- | --- |
| OpenAI | `openai` | OAuth helper via `openfox auth login --provider openai`, or `model_list[].api_keys` |
| Anthropic | `anthropic` | API key in `model_list[].api_keys`, or helper flow via `openfox auth login --provider anthropic` |
...
OpenFox also carries first-class metadata or routing support for additional vendors such as:
...
OpenFox tools are configured under `tools` in `config.json` and registered dynamically at runtime.
...
OpenFox has a first-class subagent model for long-running work, isolated subproblems, and multi-agent specialization.
...
When an agent has the `spawn` tool and at least one allowed peer, OpenFox injects a lightweight agent registry into the system prompt automatically.
...
```text
spawn(
  task="Search the web for the latest OpenFox MCP integration patterns and summarize them.",
  label="mcp-research",
  agent_id="research"
)
```
...
OpenFox can transcribe inbound audio and synthesize outbound speech, but voice setup is model-driven like the rest of the runtime.
...
| ElevenLabs Scribe | `provider: elevenlabs`, `model: scribe_v1` | Uses OpenFox's dedicated ElevenLabs transcription path |
...
- if it resolves to an ElevenLabs model, OpenFox uses the ElevenLabs transcriber
- if it resolves to a Whisper-compatible model, OpenFox uses the Whisper transcription path
- if it resolves to an audio-capable multimodal model, OpenFox can use audio-model transcription
- if `voice.model_name` is omitted, OpenFox still performs compatibility scanning across `model_list` for legacy auto-detected ASR entries
...
| --- | --- | --- |
| OpenAI-compatible speech | `openai/tts-1` | Best-supported path; OpenFox sends an OpenAI-style `/audio/speech` request |
...
- if that is missing, OpenFox can still scan `model_list` for the first API-backed model whose ID contains `tts`
...
OpenFox's shared slash command registry lives under `pkg/commands`.
...
```text
~/.openfox/config.json          Main config
~/.openfox/.security.yml       Sensitive values stored outside config.json
~/.openfox/auth.json           OAuth/token store
~/.openfox/workspace/          Default workspace
~/.openfox/workspace/skills/   Workspace skills
~/.openfox/workspace/sessions/ Session history
~/.openfox/workspace/cron/     Scheduled jobs store
```
...
```bash
OPENFOX_CONFIG=/path/to/config.json
OPENFOX_HOME=/path/to/openfox-home
OPENFOX_BUILTIN_SKILLS=/path/to/custom-builtin-skills
OPENFOX_LOG_LEVEL=debug
OPENFOX_GATEWAY_HOST=0.0.0.0
```

Use `OPENFOX_CONFIG` when the user reports "wrong config file" behavior.
Use `OPENFOX_HOME` when the user wants a portable or service-managed install.

## OpenFox-Native Concepts

### Model Configuration

OpenFox is model-centric. The key fields are:
...
- If `provider` is set, OpenFox sends `model` to that provider unchanged.
...
OpenFox only relies on `name` and `description` frontmatter fields for loading and matching.

### MCP Discovery

OpenFox supports always-loaded and deferred MCP tools.

Use:

- `--deferred` when tools should stay hidden until explicitly discovered
- `openfox mcp show <name>` to inspect active tools
...
Start with the most OpenFox-native path:

1. Check `openfox status`.
2. Confirm which config file is active.
3. Inspect `agents.defaults.model_name` and `model_list`.
4. Run `openfox gateway --debug` for runtime visibility.
...
7. For MCP issues, use `openfox mcp list`, `show`, and `test`.
...
For gateway and runtime debugging, OpenFox writes logs under its home directory:

```text
~/.openfox/logs/gateway.log
~/.openfox/logs/gateway_panic.log
```

If `OPENFOX_HOME` is overridden, use:

```text
$OPENFOX_HOME/logs/gateway.log
$OPENFOX_HOME/logs/gateway_panic.log
```

In practice, check these places first:

- `~/.openfox/logs/` for persisted gateway logs
- the terminal running `openfox gateway` or `openfox agent`
- Docker stdout/stderr via `docker compose -f docker/docker-compose.yml logs -f`
- launcher or service logs if OpenFox is being run under another supervisor

Useful controls:

- `openfox gateway --debug` for detailed runtime logs
- `openfox gateway --debug --no-truncate` for full prompt/tool payload inspection
- `gateway.log_level` or `OPENFOX_LOG_LEVEL` to raise verbosity to `debug` or `info`

Special case:

- the standalone `openfox agent` command uses console logging unless `OPENFOX_LOG_FILE` is set explicitly
- process hooks can write JSONL file logs when `OPENFOX_HOOK_LOG_FILE` is set; this is hook-specific and separate from the main gateway log path

## Repository Map

When contributing code, these paths matter most:

- `cmd/openfox/main.go` — root CLI wiring
- `cmd/openfox/internal/agent/` — direct CLI agent command
- `cmd/openfox/internal/gateway/` — gateway startup and flags
- `cmd/openfox/internal/auth/` — auth flows, QR onboarding
- `cmd/openfox/internal/model/` — default model switching and `model add`
- `cmd/openfox/internal/skills/` — install/list/show/remove/search commands
- `cmd/openfox/internal/mcp/` — MCP CLI configuration manager
- `cmd/openfox/internal/cron/` — cron CLI
...
- the `provider` and `model` fields use OpenFox's model-centric rules
...
- `openfox mcp test <name>` succeeds
- `/show mcp <server>` or `openfox mcp show <name>` exposes tools
...
If the task is code-level rather than user-facing, read the matching package under `cmd/openfox/internal/`, `pkg/commands/`, `pkg/agent/`, `pkg/skills/`, or `pkg/mcp/` before proposing behavior changes.
