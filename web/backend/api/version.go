...
var (
	...
	findOpenfoxBinaryForInfo    = resolveGatewayBinaryForVersionInfo
	runOpenfoxVersionOutput     = executeOpenfoxVersion
...
// resolveSystemVersionInfo prefers the actual openfox binary version output,
...
func (h *Handler) resolveSystemVersionInfoUncached(ctx context.Context) systemVersionResponse {
	...
	execPath := strings.TrimSpace(findOpenfoxBinaryForInfo())
	if execPath == "" {
		return fallback
	}
	...
	output, err := runOpenfoxVersionOutput(cmdCtx, execPath)
	if err != nil {
		return fallback
	}

	parsed, ok := parseOpenfoxVersionOutput(output)
	if !ok {
		return fallback
	}
...
func resolveGatewayBinaryForVersionInfo() string {
	...
	return utils.FindOpenfoxBinary()
}
...
// executeOpenfoxVersion runs the version subcommand against the
// discovered openfox executable.
func executeOpenfoxVersion(ctx context.Context, execPath string) (string, error) {
...
// parseOpenfoxVersionOutput extracts version/build/go fields from CLI output.
// It accepts banner/ANSI-decorated output and only requires the version line.
func parseOpenfoxVersionOutput(raw string) (systemVersionResponse, bool) {
