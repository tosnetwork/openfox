...
func profile() MarketProfile {
	return MarketProfile{GlobalID: 42, WorkchainID: -1, MarketAddress: rawAddress(0x11),
...
func signedOrder(t *testing.T, private ed25519.PrivateKey, owner string, nonce uint64,
	...
	price uint16, quantity uint64) ([]byte, protocol.Hash32) {
	t.Helper()
	order := protocol.PredictionOrderV1{GlobalID: 42, WorkchainID: -1,
...
func snapshot(owner string, private ed25519.PrivateKey, free, yes, no uint64) ChainAccountSnapshot {
	...
	return ChainAccountSnapshot{OwnerAddress: owner, TradingPublicKey: public, KeyEpoch: 3,
...
func TestBookAdmissionPlanningAndCrashRecovery(t *testing.T) {
	...
	book, err := OpenBook(directory, profile())
	if err != nil {
		t.Fatal(err)
	}
	...
	leftBOC, leftDigest := signedOrder(t, keyA, ownerA, 1, protocol.ActionBuy, protocol.OutcomeYes, protocol.RoleMaker, 6_000, 10)
	leftSnapshot := snapshot(ownerA, keyA, 6*testTOS, 0, 0)
	left, err := book.Admit(leftBOC, leftSnapshot, 10_001)
	if err != nil || left.Digest != leftDigest.CellHashString() {
		t.Fatalf("admit maker: record=%+v err=%v", left, err)
	}
	if retry, err := book.Admit(leftBOC, leftSnapshot, 10_001); err != nil || retry.Digest != left.Digest {
		t.Fatalf("exact admission retry was not idempotent: %v", err)
	}
	...
	rightBOC, rightDigest := signedOrder(t, keyB, ownerB, 1, protocol.ActionBuy, protocol.OutcomeNo, protocol.RoleTaker, 4_000, 10)
	rightSnapshot := snapshot(ownerB, keyB, 4*testTOS, 0, 0)
	if _, err := book.Admit(rightBOC, rightSnapshot, 10_001); err != nil {
		t.Fatal(err)
	}
	marketSnapshot := ChainMarketSnapshot{Accounts: map[string]protocol.AccountBalance{
		ownerA: {Free: 6 * testTOS}, ownerB: {Free: 4 * testTOS}}, ObservedAt: 10_000,
	...
	plan, err := book.PlanMatch(leftDigest.CellHashString(), rightDigest.CellHashString(), 10, 10_001,
		marketSnapshot, leftSnapshot, rightSnapshot)
	if err != nil || plan.Amounts.Notional != 10*testTOS || plan.Amounts.YesValue != 6*testTOS ||
		string(plan.LeftBOC) != string(leftBOC) || string(plan.RightBOC) != string(rightBOC) {
		t.Fatalf("unexpected exact match plan: %+v err=%v", plan, err)
	}
	...
	book, err = OpenBook(directory, profile())
	if err != nil {
		t.Fatalf("crash recovery rejected valid durable state: %v", err)
	}
...
func TestBookRejectsTamperedDurableSignedBytes(t *testing.T) {
	...
	index := strings.Index(string(raw), string(needle)) + len(needle)
	if index < len(needle) {
