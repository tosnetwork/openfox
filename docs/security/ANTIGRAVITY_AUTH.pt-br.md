...
**Antigravity** (Google Cloud Code Assist) é um provedor de modelos de IA apoiado pelo Google que oferece acesso a modelos como Claude Opus 4.6 e Gemini através da infraestrutura de nuvem do Google. Este documento fornece um guia completo sobre como a autenticação funciona, como buscar modelos e como implementar um novo provedor no OpenFox.
...
10. [Criando um Novo Provedor no OpenFox](#criando-um-novo-provedor-no-openfox)
...
Os perfis de autenticação são armazenados em `~/.openfox/auth.json`:
...
## Criando um Novo Provedor no OpenFox

Os provedores do OpenFox são implementados como pacotes Go em `pkg/providers/`. Para adicionar um novo provedor:
...
Se seu provedor requer OAuth ou autenticação especial, adicione um caso em `cmd/openfox/internal/auth/helpers.go`:
...
- **Arquivos Fonte:**
  ...
  - `pkg/auth/store.go` - Armazenamento de credenciais de autenticação (`~/.openfox/auth.json`)
  ...
  - `cmd/openfox/internal/auth/helpers.go` - Comandos CLI de autenticação
...
- Atualizar tokens OAuth: `openfox auth login --provider antigravity`
...
- Verificar o armazenamento do perfil de autenticação: `~/.openfox/auth.json`
- Executar novamente `openfox auth login --provider antigravity`
